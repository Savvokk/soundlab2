...
sound = wave.open(name, mode="rb")#битное чтение
frame_count = sound.getnframes()
print(frame_count)
data = sound.readframes(frame_count)# считаем кол во фреймов
params = sound.getparams()
sound.close()

data = struct.unpack("<" + str(frame_count * 2) + "h", data)#распаковка данных
new_data = list(data)# запись в массив
